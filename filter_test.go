...
func TestPatternFilter_Filter(t *testing.T) {
	...
	lines := []struct {
		name  string
		isdir bool
		want  []gitrim.FilterResult
	}{
		{
			"aptos/test/test.js",
			false,
			[]gitrim.FilterResult{gitrim.FilterResult_In, gitrim.FilterResult_Out},
		},
		{
			"aptos/test/src/abc.rs",
			false,
			[]gitrim.FilterResult{gitrim.FilterResult_Out, gitrim.FilterResult_In},
		},
	}

	filters := make([]*gitrim.PatternFilter, 0, len(patterns))

	for _, p := range patterns {
		f, err := gitrim.NewPatternFilter(p)
		if err != nil {
			t.Fatal(err)
		}
...
func TestPatternFilter_Filter_many(t *testing.T) {
	...
	for _, ap := range patterns {
		f, err := gitrim.NewPatternFilter(ap)
		if err != nil {
			t.Error(err)
		}
		...
		for _, line := range filelines {
			...
			if r == gitignore.Exclude && fr != gitrim.FilterResult_In {
				t.Errorf("gitignore says %s but we says %s for pattern %s and line %s", "in", fr.String(), ap, line)
			} else if r == gitignore.NoMatch && fr != gitrim.FilterResult_Out {
